...
test, all co-located in a folder-per-component layout. Future components then follow
this exact, verified pattern.

## 2. In scope

- shadcn initialized + preset applied to `client/` (Vite, **not** Next.js).
- One Button component from shadcn, in a **folder-per-component** layout under
  `client/src/components/ui/Button/` (PascalCase).
- Co-located in that folder: `Button.test.tsx` (Vitest), `Button.stories.tsx`
  (Storybook v10), `Button.vr.ts` (Playwright VR).
...
- Full CI wiring of VR snapshots (cross-platform baseline strategy is **documented** in
  `design.md`; the CI job is an optional follow-up).
...
| **R2**  | shadcn config matches repo conventions: `tailwind.css` → `src/styles.css`; aliases use the existing `#/` prefix (add a tsconfig `paths` mirror only if the CLI forces it).                                                                    | `components.json` shows `src/styles.css` + `#/` aliases; generated imports resolve; typecheck passes. |
...
| **R4**  | Button added via shadcn, then placed at `components/ui/Button/Button.tsx` (PascalCase folder); renders all preset variants + sizes.                                                                                                           | Button visible on a demo route/story in both themes.                                                  |
...
| **R7**  | Storybook v10 + `@storybook/react-vite` installed; Tailwind v4 wired (`viteFinal` + `src/styles.css` imported in preview). Default `src/stories/` example **removed** (layout guard).                                                         | `pnpm storybook` serves; `pnpm build-storybook` succeeds; tokens visible; no `stories/` dir.          |
| **R8**  | Playwright installed; co-located **`Button.vr.ts`** (no `.spec`/`.test` → Vitest's default globs ignore it; orphan-test rule never fires). `testMatch: **/*.vr.{ts,tsx}`. Opens the Button story, `toHaveScreenshot()` for key states/themes. | `pnpm test:vr` green vs committed baselines.                                                          |
...
| **R11** | Co-location passes the layout guard: no `__tests__`/`stories/` dirs (Storybook example removed); `Button.vr.ts` not matched by Vitest; every test has a source sibling.                                                                       | `bash tooling/check-layout.sh` passes; pre-commit clean.                                              |

## 5. Gray areas (resolved in `design.md`)

- **GA-1 — alias prefix:** keep `#/` (repo convention) vs add `@/` (shadcn default).
  Resolved: `#/`; flip only if the CLI hard-requires `@/`.
...
- **GA-3 — preset palette vs brand:** confirm `b5claE9qM` colors against the
  teal/purple guardrail after decode (R3) — show the user before applying (hard gate).
