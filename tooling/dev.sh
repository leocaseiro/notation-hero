...
if tmux has-session -t "=$SESSION" 2>/dev/null; then
  echo "Attaching to the existing '$SESSION' session (kill it with: tmux kill-session -t $SESSION)."
  exec tmux attach-session -t "=$SESSION"
fi

tmux new-session -d -s "$SESSION" -n apps -c "$ROOT" "$server_pane"
tmux split-window -h -t "$SESSION:apps" -c "$ROOT" "$web_pane"
