...
// axe-core pass per Bpm story, in BOTH themes and in BOTH the resting and hover states.
// Story IDs come from the shared Bpm.story-ids list (lockstep with VR + Bpm.stories.tsx).
// Theme is driven via Storybook's `globals` query param → our preview decorator's `.dark`
// class, so axe sees the real rendered colors.
const themes = ['light', 'dark'] as const;

for (const theme of themes) {
  for (const story of BPM_STORY_IDS) {
    test(`Bpm / ${story} / ${theme}`, async ({ page }) => {
      await page.goto(`/iframe.html?id=ui-bpm--${story}&viewMode=story&globals=theme:${theme}`);
      await page.locator('#storybook-root').waitFor();
      await page.locator('[data-slot="bpm"]').first().waitFor();
      if (theme === 'dark') {
      ...
      // Only the `range` story renders a Material Symbols glyph (the ramp arrow);
      // fonts.ready resolves even on load FAILURE, which would leave ligature fallback
      // text. Assert the icon font actually loaded for that story so a failed load can't
      // pass as fallback text.
      if (story === 'range') {
      ...
      await page.locator('[data-slot="bpm"]').first().hover();
