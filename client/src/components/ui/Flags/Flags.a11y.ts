...
// axe-core pass per Flags story, in BOTH themes and in BOTH the resting and hover
// states. Story IDs come from the shared Flags.story-ids list (lockstep with VR +
// Flags.stories.tsx). Theme is driven via Storybook's `globals` query param → our
// preview decorator's `.dark` class, so axe sees the real rendered colors.
const themes = ['light', 'dark'] as const;

for (const theme of themes) {
  for (const story of FLAGS_STORY_IDS) {
    test(`Flags / ${story} / ${theme}`, async ({ page }) => {
      await page.goto(`/iframe.html?id=ui-flags--${story}&viewMode=story&globals=theme:${theme}`);
      await page.locator('#storybook-root').waitFor();
      await page.locator('[data-slot="flags"]').first().waitFor();
      if (theme === 'dark') {
      ...
      // Flags renders a Material Symbols glyph in EVERY story; fonts.ready resolves even
      ...
      await page.locator('[data-slot="flags"]').first().hover();
