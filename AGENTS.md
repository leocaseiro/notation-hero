...
right after checkout (the composite handles the rest).
...
CI composite (`.github/actions/setup-js`) reads the same `.nvmrc`, so local
and CI Node versions stay in sync from one file.
