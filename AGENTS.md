...
- **Now building (v0, NH-291)** — a **local-file drum player** in `web/`: open a chart from disk, see drum notation, press play. The **catalog**, the backend (Neon, Cognito) and the Playable schema are **paused, not dropped** (2026-09-12 registry entry). Offline is a later milestone: v0 installs as a PWA but needs a network.
