...
| Q2  | **Answered (2026-09-16): Vercel serves `public/alphatab/esm/*.mjs` as `application/javascript; charset=utf-8`, and `/spike/esm` reports `Environment.webPlatform = BrowserModule` on the deployed preview — D5 holds. This check needed Vercel's Preview Deployment Protection disabled first, since it otherwise redirects unauthenticated requests to Vercel's SSO login before they reach the app — Task 2 Step 11 and Task 14 hit the same requirement.** CDN compression for `.sf3` is a cost question, not a correctness one, and still waits for the v0 deploy.                                                                  | `.sf3` compression: v0 deploy            |
