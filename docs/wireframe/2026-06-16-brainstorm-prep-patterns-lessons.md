...
- **"A Lesson = 1+ patterns."** You don't browse loose patterns — **Lessons *is* the browse**:
...
1. **Is a STEP the same as a PATTERN, or different?** Lesson = ordered steps — is each *step* a pattern, or
   does a step *reference* a pattern? → reconcile `exercise` vs `pattern` (maybe step→pattern link, maybe merge).
...
3. **GP-file role (flagged flaw — SD-23):** a GuitarPro file can be a **song** *or* a **pattern/groove**.
...
6. **Song-breakdowns** are reached **through the song** ("Practice in parts"), *not* listed under Lessons —
