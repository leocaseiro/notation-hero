...
function extract(path) {
  ...
  const sections = [];
  mb.forEach((b, i) => {
    if (b.isSectionStart && b.section)
      sections.push({ label: b.section.text || '', startBar: i + 1 });
  });
  ...
  const fills = bars
    ...
    .map((b) => ({
      ...
      sig: b.sig,
      tomHits: b.tomHits,
      preSection: sectionStarts.has(b.bar + 1),
    }));
  ...
  return {
    ...
    distinctBars: grooves.length,
    grooves: grooves.slice(0, 12),
...
out.forEach((o) =>
  o.grooves?.forEach((g) => {
    (bySig[g.sig] ??= []).push({ file: o.file, count: g.count });
  }),
);
