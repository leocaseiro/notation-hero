...
- **One shape:** a **`playable`** is a _leaf_ (its own `notation`/score) **or** a _sequence_ (ordered `step`s → other playables) — or both.
...
4. **Per-playable scores** everywhere (lesson whole-play score _and_ per-step/pattern scores; no averaging).
