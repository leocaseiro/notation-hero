...
- **Thin model** (Neon = browse/search metadata + file keys; AlphaTab owns score internals) and the **hybrid** rule (hot facet = column; long-tail = jsonb) are intact — the wireframe should *show* them, not fight them.
