...
| **D-3**     | Per-instrument difficulty | **3 layers.** L1 `playable.level` (browse headline, derivable). L2 `track.level` (per-instrument; a track _is_ one instrument). L3 `data.sections[i].tracks[]` (per-section × per-track grid `{track, level, techniques[]}`). Curve axes renamed: `by:'fingering'`→**`by:'technique'`**, `by:'bpm'`→**`by:'tempo'`**.   |
