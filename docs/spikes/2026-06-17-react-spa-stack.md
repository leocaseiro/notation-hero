...
| Offline store | ~~RxDB~~ → **plain Dexie** _(superseded; see offline spike)_ | dexie 4.4.4     |
| Typed API     | ~~ts-rest~~ → **oRPC** _(superseded; see contract spike)_    | @orpc/\* 1.14.x |
...
- **TanStack Router over React Router v7** — typed routes + typed search params (good for `/play?songId=&difficulty=&speed=`), same ecosystem as Query/oRPC. _Closest call of the session;_ RR v7 (data mode) is the legitimate "use-what-I-know, focus-on-AWS" alternative.
