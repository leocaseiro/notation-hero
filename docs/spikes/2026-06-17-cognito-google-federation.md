...
- **G6 refresh-token rotation** ON (best practice for SPAs) → old `REFRESH_TOKEN_AUTH` flow unavailable; refresh via `/oauth2/token grant_type=refresh_token`. Google's 7-day test-user limit doesn't bite (you hold _Cognito_ refresh tokens, 30 days).
