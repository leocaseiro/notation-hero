...
**Keep Drizzle** (`drizzle-orm/neon-http` + `drizzle-kit` + `drizzle-zod`). Decisive: _only true ORM that natively rides Neon's HTTP driver, has zero SWC/decorator friction, and lets the raw SQL DDL stay source-of-truth._ `@nestjs/typeorm` wins only the "first-party" axis — least relevant under a hexagon — and loses Neon-HTTP + SWC. **"TypeORM is abandoned" is now stale** (governed 1.0 shipped May 2026) but it still loses this stack.
