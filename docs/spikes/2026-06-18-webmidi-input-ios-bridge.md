...
- **Knock-on (the killer):** **Tone.js itself won't run** on that ancient engine — Leo confirmed this **empirically** (tried it, it broke). Tone.js is a modern **Web Audio / ES2015+** library (built on `standardized-audio-context`), so **even polyfilling the MIDI `Array.from` wouldn't save you** — the audio engine needs the modern engine broadly. Plus the shim requires users to install a specific third-party browser app (not a real installable PWA).
