...
The "let the generator pre-wire everything" benefit **does not exist on a router-only SPA**. In `@tanstack/cli` 0.69.3, `--router-only` **drops all `--add-ons` by design** (source: `command-line.ts` — _"Ignoring --add-ons in router-only compatibility mode"_; matches our Phase 0 observation). The bundled generator pipeline only comes with **TanStack Start (SSR)**.

But **all ten tools have plain-Vite-SPA paths**, and three have their _own_ one-command CLIs (Storybook, Shadcn, Sentry) — so "pipeline pre-wired" still holds, sourced per-tool.
...
**Decisive trade-off:** the value is "one generator wires everything" — but that pipeline isn't available on the SPA at all; it only comes with Start, which costs either the $0 lock or a migration+bug. Per-tool inits deliver the same wired outcome on the SPA at low effort. → **Stay SPA.** Flip only if you later want SSR for _other_ reasons (SEO), a separate decision.
