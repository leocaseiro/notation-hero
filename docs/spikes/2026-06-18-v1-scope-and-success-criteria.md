...
3  - primary by Roland DT-1 V-Drums Tutor (discountined)
4  - also inspired by Melodics
5  - Clone Hero / Guitar hero / Rock band
6  - Synthesia
7
8  I want to create an app to learn and practice how to play drums.
9  As a main goal, the app should have all the features from the DT-1 V-Drums Tutor software from Roland which is discontinued.
10 - The app should:
...
18     - display primary the song in notation, but a nice to have would be to display the song in a friendly notation view (like Synthesia as falling notes or Melodics as horizontal friendly notation for users that are not familiar with notation).
...
### v1.5 — "the $2 app push"
- Android Capacitor build at parity with iPad. *(see re-verify: stack-era assumption)*
...
- **Spike-not-architecture framing** (prior-art assignment): *"The spike is the assignment, not the architecture. Don't draw a database schema until you've felt a drum hit make a ring appear."*
- **"Unusually deep spec" note**: *"Scope.md covers feature flags Melodics doesn't have (memory mode, auto-speed practice, hi-hat pedal extra-hit forgiveness, dynamic detection for ghost notes)."*
