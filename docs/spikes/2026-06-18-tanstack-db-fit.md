...
1. **Maturity — HIGH risk for a foundation.** Core `@tanstack/db` is **0.6.8, BETA, no 1.0** (repo created 2025-03-11; ~100 releases in 2 months). Durable persistence is a **first-alpha SQLite-WASM** layer (March 2026). _Version trap:_ `@tanstack/query-db-collection 1.0.40` is a separate adapter, not the core lib.
