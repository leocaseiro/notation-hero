...
interface CatalogueItem {      // Lesson | Song — distinct types, shared base columns
  lessonId: string;            // uuid (catalogue item id)
...
The logical record above maps to one `catalogue` row. **Promote queried fields to typed columns**; keep **variable/nested/type-specific content in a `data jsonb` column** (this is where `meta`, Song `parts`/sections, per-section tags, and late `/design-shotgun` findings live — no migration needed).
...
create index on catalogue (status, category, sort_order);   -- library listing
create index on catalogue using gin (tags);                 -- "items with tag X"
create index on catalogue using gin (data jsonb_path_ops);  -- document-style queries on JSONB
create index on catalogue using gin (title gin_trgm_ops);   -- partial/fuzzy title (LIKE '%x%')
create index on catalogue (artist);
create index on catalogue (difficulty);
create index on catalogue (updated_at desc);                -- recency / future change-feed
```
...
select * from catalogue where title ilike '%love%';                            -- partial (pg_trgm-indexed)
select * from catalogue where data @> '{"focus":"double-bass"}';               -- document-style on JSONB
```

**Song `parts` (later):** relational `song_parts` table (FK → `catalogue`, queryable) **or** embed under `data->'parts'` (read-as-unit) — decide embed-vs-reference when the feature lands.

**Per-user data lives in DynamoDB** (scores / settings / mappings / sync) — the shared catalogue and per-user data are a clean seam across two stores.
...
- **Shared data, no identity** (per the locked sync model): the catalogue is global/curated → stored in **Neon Postgres** (per-user data is separate, in **DynamoDB** — a clean two-store seam). User-uploaded songs at M1 add per-user catalogue rows (keyed by uploader); per-user *sync* data stays in DynamoDB.
...
3. **Lesson vs Song:** ✅ **RESOLVED (2026-06-09)** — *distinct* entities (different schemas): one `catalogue` table with a `type` discriminator + shared base columns; type-specific structure (Song `parts`) lives in `data jsonb` / related tables. Songs gain `parts`/sections **later**.
