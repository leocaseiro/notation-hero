...
- [ ] **`[Step 5]` Configure Renovate:** set `minimumReleaseAge: '7 days'` (raised from 3 to match the pnpm SAST floor ≥ 7 days, NH-259); restrict automerge to `lockFileMaintenance` PRs only; all dependency version bumps (including patches) require human review (per L9 hardening).
