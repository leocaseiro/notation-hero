...
### From the 2026-06-17/18 expert review (set aside for your decision)

Surfaced by the ce-doc-review panel (NH-194). Not yet actioned — decide during the DACI/registry rewrite (W2) or before implementation. (The panel's safe, unambiguous fixes were already applied; these are the judgment calls.)

- **SCOPE-1 (P1) — R13–R16 v1 scope.** `POST /sync/batch` (R14) and `DEFERRABLE` foreign keys (R16) are built for the offline path, which is stubbed until M1. Consider splitting the schema-only seams (R13 client ULID primary keys, R15 `upload_status` — cheap) from the server work (R14, R16) and deferring R14 to M1.
- **SCOPE-4 (P2) — depcruise canary as a required gate.** Promote the core-purity canary (a deliberate `import '@nestjs/common'` in `core/` that must fail the build) from advice to a blocking CI check, so the hexagon fence is verified after the migration rewrites the depcruise config.
- **SEC-4 (P2) — AlphaTab WASM and the CSP.** Before enforcing the CSP, confirm whether AlphaTab ships a WebAssembly build; if it does, decide the compensating controls before adding `wasm-unsafe-eval` to `script-src`.
- **PROD-1 (P2) — dropped server-side rendering as a portfolio signal.** The superseded Next.js ADR named SSR as a showcase; ARCH-FE-1 drops it. *(Leo's note, 2026-06-18: a recruiter cannot evaluate an API without a UI, so the SPA itself is the portfolio signal; only SSR is dropped, and the visible interface remains. Treat this as accepted unless revisited.)*
- **ADV-1 (P2) — "cheapest moment" framing.** The foundation (Nx wiring across ~19 commits and several PRs) is itself a shipped, load-bearing artifact. Reframe the justification from "almost no code shipped" to "removal is real work, but cheaper now than after more features land," and state the do-nothing option explicitly.
- **ADV-2 (P2) — Nx-friction reasoning.** The stated friction was "Nx + pnpm + generators," but pnpm and generators are kept. Lead with the evidenced argument ("Nx earns nothing today") rather than the causal claim that Nx alone caused the friction.
- **ADV-3 (P2) — "no conflicts by construction" wording.** Settings use last-write-wins (a conflict resolution that discards the older write), and offline edits are forbidden (online-first) rather than impossible. State that conflicts are avoided by the insert-only rule, with settings LWW as the one accepted exception.
- **ADV-4 (P2) — eslint-boundaries migration scope.** Treat the `eslint-plugin-boundaries` change as re-deriving the element model for the `server/src` layout (not only re-pointing paths), and extend the canary check to it.
