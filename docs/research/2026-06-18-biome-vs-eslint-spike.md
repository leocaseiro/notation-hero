...
- **Honest correction (caught by the adversarial verifier):** Biome **does** ship the *basic* React hook rules on by default — `useHookAtTopLevel` (= rules-of-hooks) and `useExhaustiveDependencies` (= exhaustive-deps). So the real ESLint-only advantage on the client is **React Compiler diagnostics (~14 rules) + the TanStack plugins** — **not** basic hook safety. Still decisively ESLint for the client, but for the narrower/correct reason.
