...
| **Write/admin auth** | **AWS Cognito** user pool (provisioned via Pulumi — keeps Pulumi the single IaC, per NH-185). An **admin group** gates writes (`POST/PUT/DELETE`) + admin-read (all statuses); the same pool enables end-user auth later. The Lambda verifies the Cognito **JWT** (cached JWKS) in a `preHandler`. Public reads forced to `status='published'`. **Design → spike `docs/spikes/2026-06-17-catalogue-cognito-auth.md`** | The curated CMS needs admin-gated writes (registry F1); **Cognito is the real auth — no interim admin shortcut**. Cognito free tier (50k MAUs) is **perpetual $0** — no Amplify, no migration deadline; resolves NH-185 "no Amplify, Pulumi single IaC" cleanly |
...
| 9 | Open URL exposes **writes** (`POST/PUT/DELETE`) + draft listing (`?status=`) with **no app-layer auth** | High | **Cognito JWT** `preHandler` (verify cached JWKS + admin group) on write + admin-read routes (Task CU.4 + auth spike); public reads force `status='published'` (Task R.10) — per registry F1 |
...
- [ ] **Step 0 (admin gate via Cognito):** Add a Cognito-JWT `preHandler` (`cognito-auth.service.ts`) on `POST/PUT/DELETE /catalogue[...]` (and the admin-read path): verify `Authorization: Bearer <jwt>` against the user pool's **cached JWKS** and require the **admin group** claim; `401` (no/invalid token) / `403` (valid but not admin). Verification is public-key (JWKS) — **no shared secret**; pool id / client id / region are non-secret env config (Task 0.6). **Full design (JWKS caching, group claims, FE token flow) → spike `docs/spikes/2026-06-17-catalogue-cognito-auth.md`.** Test via `inject()`: no/invalid token → 401; valid non-admin → 403; valid admin → reaches the handler.
...
- **Security** — open-URL guardrails (concurrency cap + rate-limit + Budgets + validation); **writes/admin-read gated by Cognito JWT (registry F1)**; `DATABASE_URL` in **SSM SecureString + KMS**, lazy-loaded off module scope (F4/F11); all SQL parameterized (`list-filter.mapper.ts`). ✅
