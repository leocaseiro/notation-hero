...
- [ ] One-time (outside CI):
  ...
  - **Migrate existing stack state (F18):** the current stack uses the LOCAL/default Pulumi backend (`infra/Pulumi.yaml` has no `cloud-url`). **If `handler-hello` has already been `pulumi up`'d**, then before the first CI `pulumi up` against `s3://<state-bucket>` you must `pulumi stack export` from the local backend and `pulumi stack import` into the S3-backed stack — otherwise Pulumi sees an EMPTY backend and tries to re-CREATE handler-hello's existing resources (incl. the deterministic LogGroup `/aws/lambda/notation-hero-hello`) → "already exists" collision. (If it was never deployed, the S3 backend starts clean — no migration needed.)
