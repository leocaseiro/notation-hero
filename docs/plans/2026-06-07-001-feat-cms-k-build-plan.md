...
status: active
date: 2026-06-07
revised: 2026-06-10 # catalog store DynamoDB → Neon Postgres (Track 3 lock) — see Revision changelog
...
> **REVISED 2026-06-10 for the Postgres catalog.** Track 3 locked the catalog contract as **Neon PostgreSQL + JSONB** with a richer model (`catalog_item` / `exercise` / `pattern` / `item_pattern`) — see [specs/2026-06-10-catalog-schema.md](../specs/2026-06-10-catalog-schema.md) (**THE authoritative contract** — implement as-written, do not re-litigate fields/constraints) and [decisions/2026-06-09-catalog-store-postgres-neon.md](../decisions/2026-06-09-catalog-store-postgres-neon.md) (why Neon; why DynamoDB stays per-user-only). The original R6 trigger ("if Track 3 lands changes, update the core model") has fired: this revision swaps the **catalog data store DynamoDB → Postgres** and keeps every piece of schema-agnostic AWS plumbing (Lambdas, S3 + validator, CloudFront + OAC + KVS edge-auth, two distributions, React-Admin SPA, SNS, Pulumi components, CI + dependency-cruiser).
>
...
- **R1.** Deliver `K-1` (lesson store): S3 (`catalog/<id>/source.<ext>`) + **Neon Postgres catalog** (`catalog_item` + `exercise` + `pattern` + `item_pattern` per the locked spec) + magic-byte file validator on upload. Cover images: **out of K v1** (owner decision — RC-7): `cover_image_key` ships in the DDL but stays `NULL`; the library uses the icon fallback the spec already defines.
- **R2.** Deliver `K-3` (catalog API + delivery): Lambda Function URL behind CloudFront with `GET /v1/catalog` (the **spec §9 list projection exactly**: `id, type, title, artist, genre, level, bpm, time_sig, instruments, has_audio, has_video, sort_order, cover_image_url, status, updated_at` — excluding `data`, `notation_key`, `notation_checksum`) and `GET /v1/catalog/{id}` (full record + exercises + pattern links + short-lived signed URL for the file; song-breakdown slices resolved through a **single shared resolver** that refuses non-`published` source songs — spec §6 D2).
- **R3.** Deliver `K-2` (admin SPA + CRUD): React-Admin SPA on S3+CloudFront, gated by CloudFront Function Basic-Auth (KVS-backed credential), talking to a Lambda FURL CRUD API (catalog items + exercises + the upload routes + an explicit `POST /api/catalog/{id}/publish` enforcing the spec §5 publish gates). **Songs are created upload-first by the ingest pipeline** (round-2 review — the locked `ci_song_file` CHECK makes create-then-upload impossible for songs). Pattern-authoring/linking UI is **deferred to H-11** per spec §11; the tables + read path still ship.
...
- **R6.** Implement against **`docs/specs/2026-06-10-catalog-schema.md` (LOCKED)** — admin and player read/write the same shape. The §4 DDL + §9 indexes ship **verbatim** as the `adapters/postgres` migrations; fields/constraints are not changed by this plan.
...
- **R9.** Catalog persistence is **raw parameterized SQL** (no ORM — portability is the point; every user-supplied value bound as a parameter, never interpolated). Adapter integration tests run against **local Docker `postgres:16`**, not LocalStack.
...
- **NOT a workflow/approval CMS.** Draft/publish status exists on the `CatalogItem` record (`status` column) but no review queue, no approver routing, no notifications.
...
- **Track 3 schema lock** — ✅ **LANDED 2026-06-10** ([specs/2026-06-10-catalog-schema.md](../specs/2026-06-10-catalog-schema.md)). This revision is the response; the original "U4/U5 wait for Track 3" gate is satisfied and removed.
...
- Postgres hybrid typed-columns + JSONB catalog with `pg_trgm`/`unaccent`/tsvector search (design_pattern)
- Neon serverless driver from Lambda + the Aurora/RDS-Proxy swap story (architecture_pattern)

### External References

- **Catalog schema spec (LOCKED)** — [docs/specs/2026-06-10-catalog-schema.md](../specs/2026-06-10-catalog-schema.md) — the §4 DDL, §9 indexes/projection, §10 ingest pipeline, §5 publish gates. Validated against a live Postgres during review.
...
- **Drop `adapters/http-client/` as a separate package; collapse into `adapters/react-admin/`.** The `CatalogApiClient` is ~50-100 LOC of `fetch` boilerplate with one consumer in this plan (`catalogDataProvider`). Standalone-package overhead (workspace, package.json, version resolution hop) is unjustified. Extract when player PWA needs the same wrapper.
...
- **SNS `lesson-events` topic + admin Lambda event emit IN SCOPE for K v1** (per F-DR2b — product-lens convergence; topic name kept per the revision KEEP-list). Topic provisioned by `infra/index.ts`. Admin Lambda CRUD use-cases (U6) publish typed events (`catalog_item.created`, `catalog_item.updated`, `catalog_item.published`, `catalog_item.archived`) via `core/observability/ports/EventSink` → `adapters/sns/SnsEventSink`; the validator (U7) publishes `catalog_item.file.validated`. Event schema in `core/catalog/CatalogEvent.ts` (RC-11). NO subscribers in K — that's H-6's job. Locks the contract so H-6 can subscribe without coordinating breaking changes. SNS free tier: 1M req + 1k emails/mo.
...
- **Catalog store:** Neon Postgres + JSONB, LOCKED (decision doc 2026-06-09; spec 2026-06-10). Not re-litigated here.
...
The U1 skeleton (root configs, empty layer dirs) is **committed**. This plan creates the following tree on top of it (revised 2026-06-10 — `core/catalog/`, `adapters/postgres/`, IaC consolidated under `infra/`):

```text
notation-hero/
├── core/                                  # Pure domain — NO AWS, NO React, NO HTTP imports
│   ├── catalog/                         # Bounded context: the shared catalog (area K + later H-11)
│   │   ├── CatalogItem.ts               # Entity: song | lesson (typed facets per spec §4 ①) + Zod schema
...
│   │   ├── ids.ts                         # Branded CatalogItemId / ExerciseId / PatternId
│   │   ├── NotationFormat.ts              # 'gp'|'gpx'|'gp5'|'gp4'|'gp3'|'xml' (NO 'mid' — spec §2)
│   │   ├── FileRules.ts                   # Pure magic-byte → format decision tree + size ceilings
│   │   ├── CatalogFilter.ts             # Query language (type, level, bpm-range, …, search) per spec §9
│   │   ├── CatalogEvent.ts              # catalog_item.{created,updated,published,archived} + file.validated
...
│   │   ├── ports/
│   │   │   ├── CatalogRepository.ts     # items + exercises + pattern-links interface
│   │   │   ├── PatternRepository.ts       # pattern CRUD interface
│   │   │   ├── CatalogFileStore.ts      # mintUploadUrl/mintDeliveryUrl/promote interface
...
│   │   ├── CatalogRepositoryPostgres.ts # implements core/catalog/ports/CatalogRepository
│   │   ├── PatternRepositoryPostgres.ts   # implements core/catalog/ports/PatternRepository
│   │   ├── sql/buildListQuery.ts          # CatalogFilter → {text, params} WHERE/ORDER builder
│   │   ├── rowMappers.ts                  # snake_case row ↔ camelCase entity mapping
│   │   ├── migrations/
│   │   │   ├── 0001_catalog_init.sql    # spec §4 DDL + §9 indexes VERBATIM
...
│   ├── s3/                                # Runtime adapters
│   │   ├── CatalogFileStoreS3.ts        # implements core/catalog/ports/CatalogFileStore
│   │   ├── MagicByteValidator.ts          # implements core/catalog/ports/FileValidator
│   │   ├── __tests__/
...
│       ├── catalogDataProvider.ts       # React-Admin DataProvider (items/exercises/patterns)
│       ├── catalogResource.tsx          # Resource: catalog items (List/Edit/Create/Show + publish action)
...
│       ├── CatalogFileInput.tsx         # Custom FileInput → presigned-POST upload (source files)
...
│   │   ├── src/
│   │   │   ├── main.tsx                   # Wires CatalogApiClient + catalogDataProvider
...
flowchart TB
    ...
    subgraph "READ PLANE (public)"
        ...
        PlayerApp -->|GET /v1/catalog| PublicCF
        PublicCF --> PublicFURL
    end
    ...
    subgraph "UPLOAD PIPELINE"
        ...
        S3Catalog[(S3: catalog/&lt;id&gt;/source.&lt;ext&gt;)]
        ...
        Validator -->|valid| S3Catalog
        Validator -->|invalid incl. MIDI| S3Rejected
    end

    subgraph "STATE"
        PG[(Neon Postgres — EXTERNAL SaaS, not Pulumi-provisioned<br/>catalog_item + exercise + pattern + item_pattern<br/>pg_trgm + unaccent + tsvector search)]
        ...
        S3Catalog -.->|signed URL minted by| PublicFURL
    end
...
2. **Admin file upload:** browser POSTs to `/api/uploads` (new song) or `/api/catalog/{id}/file` (source replacement) via CF (gated path) → admin Lambda mints a **presigned S3 POST** (size-conditioned policy) → browser uploads **directly to the S3 quarantine prefix** (bypasses CloudFront entirely) → S3 ObjectCreated event triggers the validator Lambda → magic-byte sniff + §10 parse-once seeding → promote to `catalog/<id>/…` + **INSERT the draft song row** (new-song mode) or partial-`UPDATE` the existing row, then write the `status/<uploadId>.json` object the SPA polls — OR move to rejected + status (MIDI is rejected with `midi-not-renderable-convert-first`).
3. **Public read:** player app → CF (public distribution, no gate) → Lambda FURL (`AuthType=AWS_IAM` via OAC) → Postgres query (**§9 list projection** for `GET /v1/catalog`; full record + exercises + pattern links for `GET /v1/catalog/{id}`) → also mint short-lived signed S3 URL for the source file; song-breakdown slices resolve through the **shared resolver** that refuses non-`published` source songs.

The Hexagonal layer split per request: the Lambda handler is the **primary adapter** (HTTP → use case); `core/catalog/` use-cases call the **secondary adapters** (`CatalogRepositoryPostgres`, `CatalogFileStoreS3`, `MagicByteValidator`); composition root in `apps/lambda-*/handler.ts` wires them. Core never imports AWS SDK, the Neon driver, or React.
...
- Create: `core/catalog/FileRules.ts` (pure magic-byte decision tree + size ceiling: `SOURCE_MAX_BYTES = 20_000_000`)
- Create: `core/catalog/CatalogFilter.ts` (the spec §9 facet language)
- Create: `core/catalog/CatalogEvent.ts` (`catalog_item.{created,updated,published,archived}` + `catalog_item.file.validated`)
- Create: `core/catalog/publishGates.ts` (pure §5 checks)
- Create: `core/catalog/errors.ts` (`InvalidFileFormat`, `MidiNotSupported`, `ItemNotFound`, `ItemAlreadyExists`, `StaleUpdate`, `PublishGateFailed`, `SourceNotAvailable`, `ValidationError` — discriminated unions)
- Create: `core/catalog/ports/CatalogRepository.ts` · `ports/PatternRepository.ts` · `ports/CatalogFileStore.ts` · `ports/FileValidator.ts`
...
- Test: ~~`core/catalog/__tests__/{CatalogItem,Exercise,FileRules,CatalogFilter,publishGates}.test.ts`~~ <!-- SUPERSEDED: tests are CO-LOCATED next to source (Foo.ts + Foo.test.ts), NO __tests__/ folders, per tooling DACI 2026-06-09 §F-2 -->
...
```ts
// core/catalog/Exercise.ts — a lesson's ordered steps (spec §4 ②)
...
- [ ] **2.6** Write `CatalogFilter.test.ts` (limit clamp >100→100; negative limit → parse error) + `publishGates.test.ts` (lesson-no-exercises ✗ · curated-no-license ✗ · user-upload ✗ · curated+license+1-exercise ✓) → FAIL → implement → PASS → commit
...
**Verification:** ~~`pnpm vitest run core --root .`~~ <!-- SUPERSEDED: live runner is `node --test` (Node 24); Vitest deferred L5 per tooling DACI 2026-06-09 --> green with ≥95% line coverage on `core/catalog/`; `pnpm depcheck` confirms zero imports from `adapters/` or `apps/` into `core/`; `pnpm typecheck` clean. Field-for-field spot-check of `CatalogItem` against spec §4 ① at PR time.
...
**Goal:** Implement the secondary adapters that wire the core ports to real services: **`adapters/postgres/`** (NEW — `CatalogRepositoryPostgres` + `PatternRepositoryPostgres` over raw parameterized SQL, plus the **migrations** that ARE the locked DDL), `adapters/s3/` (`CatalogFileStoreS3` + `MagicByteValidator`), `adapters/sns/` (`SnsEventSink` for `CatalogEvent`). The DynamoDB catalog adapter from the original plan is **not built** (RC-3).
...
- Create: `adapters/postgres/migrations/0002_source_write_once.sql` — spec-§5-sanctioned trigger:
  ...
  BEGIN
    IF NEW.source IS DISTINCT FROM OLD.source THEN
      RAISE EXCEPTION 'catalog_item.source is write-once (set by K-1 ingest)';
    END IF;
    RETURN NEW;
  END $$;
...
- Create: `adapters/s3/CatalogFileStoreS3.ts` (implements `CatalogFileStore`: `mintUploadUrl` = S3 presigned POST; `mintDeliveryUrl` = **CloudFront signed URL** via `@aws-sdk/cloudfront-signer` (RC-13 — env: CDN domain, key-pair id, private key); `promote(quarantineKey, finalKey)` server-side copy+delete)
- Create: `adapters/s3/MagicByteValidator.ts` (wraps `core/catalog/FileRules` with `file-type` streaming)
...
- `updateItem(item, ifUnmodifiedSince)` — single statement, RC-12: `UPDATE catalog_item SET …, updated_at = now() WHERE id = $1 AND updated_at = $2 RETURNING *`; 0 rows → re-`SELECT` to distinguish `ItemNotFound` from `StaleUpdate`. **The SET list never includes `source`** (write-once; trigger backs it up) and never includes `created_at`.
- `archive(id)` → `UPDATE catalog_item SET status='archived', updated_at=now() WHERE id=$1` — the tombstone path; no `DELETE` statement exists in the adapter (spec §12).
...
- **Detail response:** the full published `CatalogItem` (sans `notationChecksum`) + `{ source_url, signed_url_expires_at }` + `exercises: [{ …step, notation: ResolvedNotation }]` (each step through `resolveStepNotation` — steps whose source is unavailable serialize as `{ notation: { kind: 'unavailable' } }` rather than failing the whole item) + `patterns: [{ id, kind, name, family, level }]`.
- **Published-only is structural:** `listCatalog` constructs the filter with `status: 'published'` (not overridable by query param); `getCatalogItem` 404s on non-published. Drafts/archived are admin-plane data.
...
- [ ] **5.4** Write `contract.test.ts` validating both wire shapes against `catalog-v1.schema.json` → FAIL → author the JSON Schema (list + detail) → PASS → commit `test(k-3): /v1 wire-contract fixtures`
...
**Goal:** Build the admin CRUD Lambda for catalog items + exercises, plus presigned-POST minting (source files) and an explicit **publish** action enforcing the spec §5 gates. Behind the gated admin CloudFront distribution with KVS-backed Basic-Auth + OAC. Implements R3.
...
- `publishItem`: loads item → gathers gate facts (`countExercises`, `unpublishedSliceSourceCount`) → `publishGates.canPublish` → on `ok`, the flip is a **single conditional statement** so the ≥1-exercise gate can't be raced by an interleaved `replaceExercises` (round-2 review): `UPDATE catalog_item SET status='published', updated_at=now() WHERE id=$1 AND updated_at=$2::timestamptz AND (type<>'lesson' OR EXISTS (SELECT 1 FROM exercise WHERE lesson_id=$1))`. Gate failures → 422 with the named gate (`{ gate: 'lesson-needs-exercise' | 'license-required' | 'curated-only' | 'song-breakdown-source-not-published' }`) so the SPA shows actionable errors. The DB CHECKs (`ci_shared_curated`, `ci_pub_license`) are the backstop — a 23514 here is a bug, logged loudly. Emits `catalog_item.published`.
- `archiveItem`: `repo.archive` → emits `catalog_item.archived`. Archived song-breakdown sources stop serving slices via U5's resolver — surfaced in the SPA confirm dialog copy ("N lessons slice this song" via a count query).
...
- [ ] **6.1** Write `handler.test.ts::createItem` — valid song 201 + event emitted; Zod failure 400 with field list; client-supplied `source:'user-upload'` ignored (row is `curated`); duplicate id → 409 → FAIL → implement `createItem` + routes → PASS → commit `feat(k-2): create catalog item + event emit`
...
- [ ] **6.3** Write `publishItem` matrix — lesson w/ 0 exercises → 422 `lesson-needs-exercise`; curated w/o license → 422 `license-required`; **lesson with a draft-source slice step → 422 `song-breakdown-source-not-published`**; happy lesson (1 exercise + license) → 200 + `catalog_item.published`; song happy path; stale `If-Match` → 412; **the conditional-flip guard: a concurrent exercise wipe between gate-check and flip leaves status unchanged** (fake repo simulates 0-rows-affected) → FAIL → implement → PASS → commit `feat(k-2): publish action with §5 gates (atomic flip)`
...
**Goal:** Build the S3-event-triggered Lambda that validates uploaded files via magic bytes, enforces the §8 streaming size ceiling, **parses the file once at upload** to seed catalog facets (spec §10.1), promotes valid files to `catalog/<id>/source.<ext>`, **INSERTs or UPDATEs the Postgres row** with file metadata + seeded facets, and routes invalid files to `rejected/` with structured reasons. MIDI is rejected, not converted (RC-5).
...
- [ ] **8.1** Write `catalogDataProvider` unit tests — `getList` filter→query-param mapping (incl. level/bpm ranges + csv arrays); `update` sends `If-Match`; 412 → `StaleUpdateError`; `publish` posts with token → FAIL → implement provider + `CatalogApiClient` → PASS → commit `feat(k-2): catalog DataProvider`
- [ ] **8.2** Write `CatalogFileInput` tests — happy multipart-POST flow stores key + enters polling; 25 MB file blocked client-side; `.mid` blocked with convert-first copy; POST network failure → inline retry; poll lands on success/rejection states → FAIL → implement → PASS → commit
- [ ] **8.3** Write `AlphaTexInput` test — valid tex passes; broken tex shows alphaTab error inline → FAIL → implement → PASS → commit
- [ ] **8.4** Build the catalog resource + exercises editor + App; smoke test renders list with mocked provider; star-mapping unit test (1→1★, 4→2★, 10→5★, null→"—"); **new-song upload dialog test: mint → POST → poll → navigate-on-success / reason-on-rejection**; switcher test: Tex→Slice switch clears tex after confirm → PASS → commit `feat(k-2): admin resources (catalog + exercises)`
...
const filesBucket = new aws.s3.Bucket('catalog-files', { /* private, AES256, versioning off, forceDestroy:false */ })
new aws.s3.BucketPublicAccessBlock('catalog-files-pab', { bucket: filesBucket.id, ...allBlocked })
new aws.s3.BucketCorsConfigurationV2('catalog-files-cors', { bucket: filesBucket.id, /* PUT/POST from adminDomain only */ })
new aws.s3.BucketLifecycleConfigurationV2('catalog-files-lifecycle', {
...
- **Neon least-privilege (runbook, not IaC):** create role `app_cms` with `SELECT/INSERT/UPDATE` on the four catalog tables + `SELECT` on `schema_migrations` (no DDL, no DELETE — the adapter never deletes); migrations run as the owning role. Document `psql` snippets in `infra/README.md`.
...
- [ ] **9.4** First real deploy: Neon project created → `pulumi config set --secret neonDatabaseUrl …` → migrate → two-pass `pulumi up` → smoke: admin Basic-Auth prompt; `GET /v1/catalog` returns `{ items: [], total: 0 }` → commit any drift fixes
...
- Error path: migrations not run before deploy → K-3 returns 503s with `relation "catalog_item" does not exist` in logs — the pre-deploy guard #3 exists precisely for this; documented in the runbook troubleshooting table.

**Verification:** `pulumi preview --stack dev` clean; `pulumi up --stack dev` succeeds; zero-diff on re-run; KVS + Neon-URL rotation drills pass; `GET /v1/catalog` 200 against the deployed stack; operator README covers Neon + migrations + all original procedures.
...
- **Unchanged invariants:** no `apps/player-pwa/` stub; player consumes Pulumi stack outputs by name. The catalog store is swappable behind K-3 (spec §12) — nothing outside `adapters/postgres/` + the `DATABASE_URL` secret knows Neon exists.
...
- **Decision-capture**: after each unit lands, capture institutional learnings via `/ce-compound` per the learnings researcher's recommendation. Highest-value: KVS Basic-Auth rotation pattern (U6/U9), `LambdaWithUrl` + OAC pattern (U3/U5/U6), Postgres hybrid-JSONB catalog + CHECK-probe migrations (U4), magic-byte streaming validation + parse-once seeding (U4/U7).
...
- **Companion docs** (all currently in worktree `pensive-boyd-6d17e3/docs/`):
  ...
  - `song-schema.md` (historical — superseded by `specs/2026-06-10-catalog-schema.md`)
...
- **Related Track plans:**
  ...
  - Track 3: ✅ landed — [specs/2026-06-10-catalog-schema.md](../specs/2026-06-10-catalog-schema.md) (this revision implements it)
...
Data-store alternatives (2026-06-10 revision): **resolved upstream, not re-litigated here** — DynamoDB-only, DynamoDB+OpenSearch, MongoDB Atlas, Supabase, Prisma Postgres, and Aurora/RDS were all weighed in [decisions/2026-06-09-catalog-store-postgres-neon.md](../decisions/2026-06-09-catalog-store-postgres-neon.md); Neon Postgres + JSONB won for the catalog, DynamoDB keeps per-user data at M1.
...
- **Public `GET /v1/catalog/{id}` returns a working signed URL** within TTL; a song-breakdown lesson resolves slices only while the source song is published.
...
- **≥5 captured solution docs in `docs/solutions/`** tied to the named patterns:
  ...
  - `docs/solutions/postgres-catalog-jsonb-search.md` — hybrid typed-columns + JSONB catalog; `pg_trgm`/`unaccent`/tsvector search; the CHECK-fidelity-probe migration pattern
...
- **1 architectural-decision capture per high-value pattern** — ADR-lite `docs/decisions/NNN-*.md` for: Hexagonal Layout 4 (+ the 6-swap Premise Audit), CF Function + KVS over Lambda@Edge + Cognito, bare FURL + OAC over API Gateway, **catalog = Postgres+JSONB over DynamoDB/Mongo (already captured: `decisions/2026-06-09-catalog-store-postgres-neon.md` — link it, don't rewrite it)**, presigned-POST direct-to-S3 over pass-through-Lambda.
...
- **`docs/solutions/`** (created post-landing via `/ce-compound`) — seed with 5+ entries per Success Metrics list (kvs-basic-auth-rotation, lambda-furl-oac-pattern, postgres-catalog-jsonb-search, neon-serverless-lambda, magic-byte-streaming-validation, pulumi-component-lambda-with-url). Plus paired whiteboard-rehearsal docs.
- **`docs/decisions/`** (created per Success Metrics) — ADR-lite captures: Hexagonal Layout 4 (+ Premise Audit), CFF+KVS vs Lambda@Edge+Cognito, bare FURL+OAC vs API Gateway, catalog Postgres+JSONB (already exists — `2026-06-09-catalog-store-postgres-neon.md`), presigned-POST direct-to-S3.
- **`adapters/postgres/README.md`** (U4) — query-pattern table for the catalog: list-with-facets (`buildListQuery`), search (trgm/unaccent/tsvector + which index serves which query), update-with-`updated_at`-check, archive tombstone, exercise batch replace; plus the migration-runner contract (verbatim-DDL rule + CHECK probes).
...
- **DR posture**: Neon free tier includes point-in-time restore (limited history) + instant branches — a pre-risky-change branch is the cheap backup (runbook snippet). S3 versioning OFF for the `catalog/` prefix (deterministic keys). Formal backups out of scope for v1.
