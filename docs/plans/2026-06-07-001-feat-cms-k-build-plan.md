...
| RC-7 | Cover images | DEFERRED (R1 narrowed to source-file-only; curator pastes a URL) | locked schema **has `cover_image_key`** → reconciled: v1 **reuses the existing quarantine/presigned-POST pipeline** for covers (JPEG/PNG/WebP magic bytes, 2 MB cap) → `catalogue/<id>/cover.<ext>`. The paste-a-URL interim is dropped |
...
- **R1.** Deliver `K-1` (lesson store): S3 (`catalogue/<id>/source.<ext>`) + **Neon Postgres catalogue** (`catalogue_item` + `exercise` + `pattern` + `item_pattern` per the locked spec) + magic-byte file validator on upload. Cover images: v1 **reuses the same quarantine/presigned-POST pipeline** (JPEG/PNG/WebP magic bytes, 2 MB cap) → `catalogue/<id>/cover.<ext>` → `cover_image_key` (RC-7).
...
- **CSP on admin SPA distribution Response Headers Policy.** `default-src 'self'`; `script-src 'self'`; `style-src 'self' 'unsafe-inline'` (MUI requires inline styles in v5); `img-src 'self' data: https:` (cover images may be on arbitrary CDNs); `connect-src 'self' https://*.s3.<region>.amazonaws.com` (presigned POST direct-to-S3). Prevents stored-XSS if React-Admin ever renders attacker-controlled content (security-lens deferred Q).
...
- **Magic-byte library:** `file-type` (MIT, streaming-capable) is the **stream-peeking plumbing + cover-image (jpg/png/webp) confirmation only** — it has no Guitar Pro signatures (a GP7/8 file detects as generic `zip`; BCFZ/gp3-5 detect as nothing). `core/catalogue/FileRules.ts` is the format authority for ALL source formats (PK/BCFZ/`FICHIER GUITAR PRO`/`<?xml`/`MThd`). (Corrected in the 2026-06-10 round-2 review — the original rationale overstated file-type's role.)
...
- **Cover images:** v1 reuses the quarantine/presigned-POST pipeline (JPEG/PNG/WebP, 2 MB) → `cover_image_key` (RC-7). The cheap alternative (curator pastes an external URL) was rejected: `cover_image_key` is an S3 key by contract, and the pipeline already exists — the marginal cost is an allowlist entry + one key shape.
...
- **Schedule tripwire numbers `[N]`/`[M]`** (Success Metrics) — owner's call.
- **`feature-freeze.md` `H-3` milestone** — still says *Alpha* for DynamoDB while RC-3 moved it to M1; which plan owns the in-window DynamoDB surface (analytics counters)?
- **Cover thumbnails: signed vs public** — carving `catalogue/*/cover.*` out of the trusted key group would let list responses cache long; covers may not be the copyright-protected asset. Spec-owner call.
- **EXIF stripping for cover images** before promotion (curator device/GPS metadata) — cheap at U7 if wanted.
...
│       ├── CatalogueFileInput.tsx         # Custom FileInput → presigned-POST upload (source + cover)
...
- `mintUploadUrl(kind, ext, itemId?)` mints a **presigned POST** via `createPresignedPost` (a plain presigned PUT cannot express a size condition — review finding): source → policy `content-length-range [0, 20_000_000]` + octet-stream/xml content types; cover → `[0, 2_000_000]` + `image/jpeg|png|webp` (RC-7). Policy fields carry `x-amz-meta-upload-id` (always) + `x-amz-meta-item-id`/`x-amz-meta-kind` (item-scoped uploads). All target `uploads/quarantine/<uuid>.<ext>`. The browser submits multipart/form-data POST (U8); bucket CORS already allows POST.
...
**Goal:** Build the admin CRUD Lambda for the **three catalogue entities** — items, exercises, patterns (+ pattern links) — plus presigned-POST minting (source files *and* covers, RC-7) and an explicit **publish** action enforcing the spec §5 gates. Behind the gated admin CloudFront distribution with KVS-backed Basic-Auth + OAC. Implements R3.
...
| `POST /api/catalogue/{id}/file` | `mintUploadUrl` | body `{ kind: 'source'|'cover', ext }` → presigned POST, item-scoped: cover upload, or source **replacement** for an existing song |
...
- `mintUploadUrl` (both routes): validates `ext` against `kind` (source: gp/gpx/gp5/gp4/gp3/xml — **mid is rejected here with the convert-first message**, before any upload happens; cover: jpg/png/webp) → presigned POST with `x-amz-meta-upload-id` (+ item id/kind when item-scoped). `getUploadStatus` reads the validator's status object (`status/<uploadId>.json` in the bucket — written on success AND failure) so the SPA's polling has something to land on (round-2 review: the 'validating…' state previously had no resolution path).
...
- [ ] **6.5** Write `mintUploadUrl` cases — non-item-scoped `{ext:'gp'}` → POST policy + quarantine key + upload-id metadata; item-scoped `{kind:'source', ext:'gp'}` and `{kind:'cover', ext:'png'}` (2 MB policy); `ext:'mid'` → 400 `midi-not-renderable-convert-first`; unknown ext → 400; `createItem` with `type:'song'` → 422 `song-created-by-upload` → FAIL → implement → PASS → commit
...
2. **Metadata:** read `x-amz-meta-upload-id` (always present) + optional `x-amz-meta-item-id`/`x-amz-meta-kind`. No upload-id → `rejected/no-metadata/<key>`. Validate ids' shape before using them in any key path (path-traversal guard). **Two modes:** item-scoped (item-id present: cover, or source replacement) vs **new-song** (no item-id, source kind — the upload-first flow from U6).
...
7. **Promote + record:** server-side copy `uploads/quarantine/<uuid>.<ext>` → `catalogue/<id>/source.<detectedExt>` (detected format wins over declared ext; mismatch logged) or `catalogue/<id>/cover.<ext>`; delete quarantine object; then write the row:
...
**Goal:** Build the React-Admin SPA over the catalogue resource (with the exercises editor nested in the lesson Edit view) — custom DataProvider, presigned-POST direct-to-S3 file inputs (new-song upload dialog + cover/replacement inputs), client-side alphaTex validation for exercise steps, and an explicit Publish action surfacing the §5 gates. Pattern-authoring UI is deferred to H-11 (spec §11; round-2 review). Deploy as static bundle to S3+CloudFront with the EdgeBasicAuth gate (unchanged).
...
- **A live public catalogue URL exists within [N] weeks of U2 start, and the whole plan lands within [M] weeks** (numbers are the owner's call — see Open Questions). When a phase overruns its share, the named descope levers fire in order: drop parse-once seeding to filename-only (U7 fallback) → defer the cover pipeline → defer the exercises editor polish. Hitting every quality metric three months late fails the actual goal.
