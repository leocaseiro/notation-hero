...
>
> Original path `docs/superpowers/specs/2026-06-19-gp-tonal-design.md`, carried on `alphatab-tonal-spike (also claude/keen-neumann-0405de)`. It never reached `master`: it was one of eight
> documents flagged as orphan-risk on 2026-06-20 and still absent three months later.
>
> **Carries five decisions marked "locked with Leo" that were never in the register.**
>
> `D-build`, `D-key`, `D-sections`, `D-naming` and `D-bars` appear nowhere under `docs/decisions/` on `master` — a grep for each of them, and for `gp-tonal`, `Krumhansl` and `NH-196`, returns zero hits. They are recorded in the decision changelog by this same PR, flagged **unverified**: recovered from a branch, still needing confirmation that they hold.
...
- **Location:** `core/gp-tonal/` in this monorepo (workspace glob `core/*`). _Note: supersedes the earlier `packages/gp-tonal/` mention — the repo uses `core/_`, not `packages/_`._
...
interface ChordHit {
  // tonal Chord.get() shape + position
  symbol: string; // 'G#m'
  tonic: string; // 'G#'
  type: string; // 'minor'
