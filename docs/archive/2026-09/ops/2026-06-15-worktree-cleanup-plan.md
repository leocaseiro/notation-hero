...
> ## ⚠️ ARCHIVED — a 2026-06-15 decision request, never answered
>
> Moved to `docs/archive/2026-09/` on **2026-09-22**, verified against `origin/master` @ `7addfd21`.
> **Do not execute the plan below.** It is a decision-request artifact, not an adopted plan: its body
> is eight open questions addressed to leocaseiro, and none of them was ever answered.
>
> **What did happen, in June:** the zero-risk batch ran — six Bucket-A worktrees removed, fourteen
> `__BKP` backups deleted (~49 MB), and NH-150 and NH-125 moved to Done. That part is history, not a
> pending action.
>
> **Why the rest is stale:**
>
> - It plans a **47 → ~7** worktree reduction. There are **39** today, and its buckets name worktrees
>   that no longer exist.
> - Its "Keep (open PRs)" list is #28, #26, #24 and #8. Only **#8** is still open — #28 and #24
>   merged, #26 was closed.
> - It leans on the KAN → NH ticket remap. The KAN project has since been drained.
>
> **What is still worth taking from it:** the Bucket A–E classification — merged + clean, merged +
> dirty, unique commits not on master, unpushed local, orphan — is the right shape for this problem,
> and a later sweep arrived at the same categories independently. Reuse the model, not the data.
...
- **Bucket A — 6/6 worktrees removed:** `competent-poitras`, `clever-mccarthy`, `determined-torvalds`,
  `musing-banach`, `mystifying-zhukovsky`, `silly-chaplygin`. All `unlanded=0`; 5 were dirty _only_ from a
...
1. **`git worktree remove` only ever risks _uncommitted_ (dirty) changes.** Every _commit_ survives
...
2. **Claude sessions live in `~/.claude/projects/…`, separate from the worktree dir** — removal never
   touches session history. _Proof:_ 3 already-removed worktrees (`condescending-elion`,
...
carry _extra_ commits on top. "Unlanded" = real new content vs `master` (3-dot diff).
...
| ☐   | `agent-aligned-enforcement`           | #25       | 8f / +1828  | _(no session — CLI/agent)_                  | Likely squash dupes — verify   |
...
| ☐   | `frosty-meninsky-0e540e`              | _none_    | 2f / +187   | "memstack mentor: KAN-119 walkthrough"      | hexagonal diagrams — PR?       |
...
| --- | ----------------------------------- | ------- | ----------- | ---------------------------------------------------------------- | --------------------------------------- | ----------------------------------------------- |
| ☐   | `agent-ad4bfc9c` / **colocate-pr8** | 9       | 33f / +2546 | **Core catalogue domain** + co-located tests (superset of PR #8) | _(no session — agent)_                  | **Push → fold into PR #8 / new PR + NH ticket** |
