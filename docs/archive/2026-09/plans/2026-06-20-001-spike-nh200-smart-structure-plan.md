...
>
> Original path `docs/plans/2026-06-20-001-spike-nh200-smart-structure-plan.md`, carried on `worktree-nh-200-smart-structure-spike`. It never reached `master`: it was one of eight
> documents flagged as orphan-risk on 2026-06-20 and still absent three months later.
>
> **The plan behind the NH-200 findings. `status: ready-for-work` — the spike ran, the product work never started.**
>
> Its frontmatter names `target_remote: private (github.com/leobraus/notation-hero-private)`, so it was drafted for a different, private repository. The content is purely technical, which is why it is safe here.
...
- **KTD-5 — Rule-based only is built; ML/LLM is written analysis.** Per NH-200, build the three rule-based methods, measure them, and only _recommend_ ML/LLM in prose with cost/complexity. No model training or LLM calls in this spike.
...
- **Test scenarios (empirical, scored in U4):**
  ...
  - On a file _with_ repeats, method (a) contributes at least the repeat boundaries.
...
- **AlphaTab repeat-structure field names.** The exact master-bar properties for repeats/alternate endings must be confirmed against the installed `@coderline/alphatab` type definitions during U3 (candidates: `isRepeatStart`, `repeatCount`, alternate-ending flags). If a field is absent, method (a) degrades to "no structural boundaries" for that file — methods (b)/(c) still run. _Execution-time verification, not a planning blocker._
