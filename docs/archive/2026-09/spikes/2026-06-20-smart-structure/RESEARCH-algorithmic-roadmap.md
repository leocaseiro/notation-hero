...
<!-- The Tier 1/2/3 list below is a single 1..9 ranking interrupted by headings, not three
     separate lists. Renumbering each run to 1/2/3 would destroy the ranking — "7. Articulation
     texture" is seventh of nine, not first of three. Archived verbatim; numbering preserved. -->

## ⚠️ ARCHIVED — recovered from a branch on 2026-09-22

> Original path `docs/spikes/2026-06-20-smart-structure/RESEARCH-algorithmic-roadmap.md`, carried on `worktree-nh-200-smart-structure-spike`. It never reached `master`: it was one of eight
> documents flagged as orphan-risk on 2026-06-20 and still absent three months later.
>
> **The most durable of the eight, and still accurate.**
>
> It sets the algorithmic ceilings (boundaries ~70-78% F1, naming ~65-80%) and names the unlock: the signals that separate verse from chorus — dynamics/velocity, polyphony, which instruments play — are all present in a Guitar Pro file and currently unused. It also names the missing method: repetition via a time-lag matrix, as opposed to local self-similarity.
...
| **Repetition**  | which segments recur (**the chorus key**) | **time-lag matrix / structure features** (Serra), SSM diagonal scoring (audio "thumbnailing"), Goto RefraiD | ❌ **no** — our SSM captures _local_ similarity, not _repetition across the song_  |
...
**Measured failure (our spike):** verse and chorus _in the same key_ have near-identical pitch-class content, so similarity-clustering lumps them together and "most-repeated class = chorus" over-applies. The literature (Goto's RefraiD: 0.938 F at _finding_ repetition but can't split verse/chorus; Maddage; ASSDP) hits the **exact same wall**.

**The fix the field uses — add an energy/texture gradient.** Music theory (Nobile's _teleological_ model) and MIR both say verse→pre-chorus→chorus is an **energy rise**, measurable from symbolic data:
...
8. **Directions / jumps** — `MasterBar.directions` (DaCapo/DalSegno/Coda/Fine) → reconstruct the _logical_ play order (we currently ignore these).
...
- **B1. Time-lag / structure-feature matrix** (Serra SF): from our per-bar histogram SSM, build an `L×B` lag matrix and run novelty on _that_ — adds the repetition principle. Expected **+5–15 F1** (MSAF benchmarks).
...
- **N2. Repetition × energy ranking** — most-repeated _and_ highest-energy recurring segment = chorus; zero-repetition short late segment = bridge; the energy-rising segment before a chorus = pre-chorus.
