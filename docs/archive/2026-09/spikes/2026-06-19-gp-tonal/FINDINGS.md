...
>
> Original path `docs/spikes/2026-06-19-gp-tonal/FINDINGS.md`, carried on `alphatab-tonal-spike (also claude/keen-neumann-0405de)`. It never reached `master`: it was one of eight
> documents flagged as orphan-risk on 2026-06-20 and still absent three months later.
>
> **Partly overtaken by shipped work.**
>
> Its core claim — AlphaTab can parse Guitar Pro and MusicXML headlessly — is now proven in production by the v0 player. What it proved beyond that (per-track Krumhansl key, tuning, sections, and a `Gp7Exporter` write-back path) is still unbuilt, so the findings keep reference value.
