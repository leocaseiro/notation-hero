...
>
> Original path `docs/spikes/2026-06-19-nh137-song-slice/FINDINGS.md`, carried on `alphatab-tonal-spike (also claude/keen-neumann-0405de)`. It never reached `master`: it was one of eight
> documents flagged as orphan-risk on 2026-06-20 and still absent three months later.
>
> **The evidence behind a decision that already shipped.**
>
> NH-137 settled on positions-only slice storage (`sourcePlayableId` + `barStart` + `barEnd`, with alphaTex and sync points derivable at runtime). This document is the four-approach comparison that led there, and its _proved by running_ versus _concluded from source_ table is the part worth keeping.
...
**Conclusion on sync:** the audio file must be stored/referenced **separately regardless of approach** — no serialization trims or re-times audio. This is a simplification, and it does **not** require splitting the audio (which the user does not want): store the full song's audio once (S3), and the slice carries only `(audioRef, barRange, msOffsetBaseline, rebasedSyncPoints[])`. At playback, feed the rebased sync points to AlphaTab and seek the shared audio element. Approach 1 _can_ re-embed the whole-song audio into each slice `.gp` (proved), but that is storage-hostile (7.93 MB per slice) and still doesn't trim the audio.
...
- **Fidelity:** Highest. GP importer heals dangling ties for free; only format that _can_ carry embedded audio.
...
### Approach 3 — Convert A–B to alphaTex (`AlphaTexExporter`), DB-only _(user leans here)_
...
- **Fidelity:** High for the common case — notation, tempo, time-sig, tuning, **chord diagrams** (`\chord ("B" 7 7 8 9 9 7)`) and **sync points** (`\sync …`) all survive. Lossy _textual_ re-encoding (rare GP-specific engraving details may lack a tex token), but everything NH needs round-tripped cleanly on all test files.
...
- **S3:** no new object _per slice_. The audio is shared **per song** — extract once (or keep inside the source `.gp`); the YouTube path needs no audio storage, just a `youtubeId` on the song.
