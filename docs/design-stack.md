# Design: Drum Tutor (Roland DT-1 Replacement) — Tech Stack Selection
...
build (e.g., DT-1's actual windows, Melodics' published windows, or
...
The user wants to build an app to learn and practice drums, modeled on Roland's
discontinued DT-1 V-Drums Tutor. Primary motivation is personal use plus a circle
...
- **Replaces an extinct tool.** Roland discontinued the DT-1 V-Drums Tutor.
  Existing owners have nowhere to go for an equivalent experience.
- **Two specific wedges vs the dominant competitor (Melodics).** Melodics
...
- **First-class custom song upload.** Drummers who already practice songs they
  love (not just licensed Melodics content) are underserved today.
- **Real-time visual feedback** with timing categories (perfect / early / late /
  missed / extra) styled after standard drum-kit notation.
- **Author dogfoods.** The doc's author is themselves a DT-1 user, currently
  stuck on legacy 32-bit Mac hardware to run it (DT-1 doesn't run on modern
  ...
  DT-1 user-forum reports consistently ask "what do I move to?" and Roland's
  only pointer is the Melodics partnership, which has the two wedge gaps
  named above.
...
- If <X% of N drummer friends use custom upload weekly in the first 30 days
  ...
  Mac/PWA-first and de-emphasize the Android Melodics-gap pitch.
...
4. Custom song upload (MIDI + Guitar Pro) is the wedge vs Melodics.
...
- **New Admin/CMS track (area `K`, Alpha):** hosted lesson manager gated by a CloudFront-Function Basic-Auth (no Cognito); produces the shared lesson library.
- **Catalogue store (2026-06-09): Neon PostgreSQL + JSONB** (hybrid: typed columns for queried fields + `data jsonb` for variable/nested incl. Song `parts`; `pg_trgm`/`tsvector` for search). **Not DynamoDB, not MongoDB** — Mongo evaluated and dropped (squeezed between Postgres-JSONB and DynamoDB; kept as a talking-point + optional local-Docker exercise). **DynamoDB stays** for per-user data + analytics counters. Lambda↔Postgres via Neon's serverless HTTP driver; AWS-managed equivalent = Aurora/RDS + RDS Proxy. See [decisions/2026-06-09-catalogue-store-postgres-neon.md](decisions/2026-06-09-catalogue-store-postgres-neon.md).
...
- 5-10 included royalty-free practice tracks (S3 files + **Neon Postgres** catalogue metadata — 2026-06-09; was DynamoDB).
...
- "Friendly notation view" (Synthesia-style falling notes) — UX needs a
...
- **You spec'd before asking, and the spec is unusually deep for a side
  project.** Scope.md covers feature flags Melodics doesn't have (memory
  ...
  who has actually used the DT-1 long enough to know what's missing.

- **You named specific competitor gaps with users-asked-for-this evidence.**
  About Melodics: *"no custom songs ... melodics users are asking"* and
  *"no Android app ... Melodics users are also asking"*. That's not "I think
...
- **Fold §6 friendly-view design into design-stack.md** — Approach A / Explicitly deferred (P0, design-lens + coherence, confidence 100)
  ...
  Implementers will build the wrong friendly view: design-stack.md describes it as "Synthesia-style falling notes" (vertical waterfall) AND defers it pending /design-consultation. But stack-brainstorm.md §6 has the complete UX spec with Melodics-style horizontal highway as primary, Synthesia vertical as optional alternate, gem shape encoding (filled / X / halo / dim for normal / cymbal / accent / ghost), translucent now-line hit-window band, velocity → brightness/size, feedback table for every event (Perfect / Early / Late / Miss / Wrong-extra with pedal hi-hat suppression), tendency meter, combo glow, and accessibility pair-color-with-shape+text+position rule. scope.md's "TBD" question is already answered in §6. User chose to brainstorm friendly-view design via `/design-shotgun` (generating multiple variants) before committing to §6's fold-in — defer the fold-in pending that pass.
...
- **'Replaces extinct tool' framing assumes addressable install base** — What Makes This Cool (P2, product-lens, 75). DT-1 has been discontinued for years; orphans have moved on. The author-dogfood evidence (added 2026-06-04) is the real wedge — reframe "extinct tool replacement" as the design north star, not a strategic claim about a waiting user base.
