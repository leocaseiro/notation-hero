...
const { CloudFrontSite, CACHE_OPTIMIZED, CACHE_DISABLED, ORP_ALL_VIEWER_EXCEPT_HOST } =
...
test('S3 bucket is private and readable only by this distribution via OAC', async () => {
  ...
  assert.ok(sourceArn && sourceArn !== '*', 'SourceArn must pin one distribution');
});
...
test('two OACs (s3 + lambda) both sign sigv4 always', async () => {
  ...
  const byType = Object.fromEntries(oacs.map((o) => [o.originAccessControlOriginType, o]));
...
test('distribution has two origins and a distinct /api/* behavior', async () => {
  ...
  assert.ok(s3Origin.originAccessControlId, 's3-spa origin must carry an OAC id');
...
test('grants CloudFront BOTH invoke permissions, pinned to the distribution', async () => {
  ...
  assert.deepEqual(actions, ['lambda:InvokeFunction', 'lambda:InvokeFunctionUrl']);
