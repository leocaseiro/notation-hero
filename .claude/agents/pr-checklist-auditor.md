...
| **error-codes**  | the diff adds a failure a person or an operator can see: a toast, an error page, user-visible failure copy, an HTTP error body, or an env/config guard                                                                                            | such a path was added but the diff changes neither `shared/src/error-codes.ts` nor `docs/reference/error-codes.md` (the CI gate only proves the registry and that page agree with _each other_ — it cannot see a new failure that was never given a code at all) |
